...
(function () {
    function x(b, c, d) {
        ...
        for (var c = c || document, d = d || "*", l = [], c = "*" === d && c.all ? c.all : c.getElementsByTagName(d), d = c.length, b = b.replace(/\-/g, "\\-"), b = RegExp("(^|\\s)" + b + "(\\s|$)"); 0 <= --d;) b.test(c[d].className) && l.push(c[d]);
        return l
    }
    ...
    function r(b, c, d) {
        b.addEventListener ?
            b.addEventListener(c, d, !1) : b.attachEvent("on" + c, d)
    }
    ...
    var i = function (b, c) {
            function d(b) {
                A(b);
                var g = {};
                if (b.offsetX && b.offsetY) g.X = b.offsetX, g.Y = b.offsetY;
                else if (b.layerX && b.layerY) g.X = b.layerX, g.Y = b.layerY;
                else return;
                if (!(20 < a.clicknum2) && (g.X -= a.offset.x, g.Y -= a.offset.y, !(0 >= g.X || 0 >= g.Y)))
                    if (!Date || !(Date.now && Date.now() - a.lastResultObj.time < a.sptime)) {
                        a.clicknum2++;
                        var c = g.X,
                            d = g.Y,
                            e = a.clicknum,
            ...
            function i(b) {
                ...
                if ("click" === b) return function (b) {
                    u.toLoading();
                    !1 !== a.onReloading() && (a.image.src = a.gp_url +
                        "&" + Math.random(), a.state = "requesting");
                    A(b)
                };
                i("click")()
            }
            ...
            function k() {
                for (var b in a.clickResult) a.imageparent.removeChild(a.clickResult[b].dom), delete a.clickResult[b]
            }
            ...
            function q(b) {
                ...
                var c, h;
                try {
                    c = this, h = c.getAttribute("index")
                } catch (d) {
                    c = window.event.srcElement, h = c.getAttribute("index")
                }
            ...
            if (!s[c]) {
                ...
                this.start = function (b) {
                    if ("object" !== typeof b) return z("touclick.st_params is not object"), this;
                    ...
                    if (a.isstart) return z("this touclick '" + this.getName() + "' has start"), this;
                    a.isstart = !0;
                    u.toLoading();
                ...
                this.setArrow = function (b, c) {
                    ...
                    if ("left" == b || "right" == b) 0 < c ? d.top = c +
                        "px" : d.bottom = Math.abs(c) + "px";
    ...
    var k = function () {
        ...
        n = function () {
            document.addEventListener ?
                (document.removeEventListener("DOMContentLoaded", n, !1), k()) : "complete" === document.readyState && (document.detachEvent("onreadystatechange", n), k())
        };
    if ("complete" === document.readyState) setTimeout(k, 1);
    else if (document.addEventListener) document.addEventListener("DOMContentLoaded", n, !1), window.addEventListener("load", k, !1), /WebKit/i.test(navigator.userAgent) && (w = setInterval(function () {
        /loaded|complete/.test(document.readyState) && k()
    }, 10));
    else {
